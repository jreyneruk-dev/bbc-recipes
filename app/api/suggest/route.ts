...
export async function POST(req: Request) {
  const { protein, effort, time } = await req.json()
...
Starter ids (dishType Starters & nibbles or Light meals & snacks): ${JSON.stringify(starters.map(r => r.id))}
Main ids (dishType Main course): ${JSON.stringify(mains.map(r => r.id))}
Snack ids (dishType Light meals & snacks or Cakes and baking): ${JSON.stringify(snacks.map(r => r.id))}

Suggest a dinner for two tonight. Preferences: protein=${protein}, effort=${effort}, time=${time} minutes.
Pick one starter, one main, optionally one snack. Only use ids from the lists above.
